...
# --- Configuration ---
st.set_page_config(page_title="Gage R&R – Dashboard Pro", layout="wide")
st.title("📊 Gage R&R – Dashboard Professionnel et Esthétique")
st.write("---")
...
if uploaded_file:
    ...
    with col1:
        ...
        st.table(main_results)
    with col2:
        st.markdown("**📌 Pourcentages (%) et NdC**")
        st.table(perc_results)

    # --- Graphiques miniatures côte à côte ---
    st.subheader("📊 Graphiques Gage R&R")
    col_box, col_hist, col_bar, col_cc = st.columns([1,1,1,1])
    ...
    with col_hist:
        ...
        for i,data in enumerate(data_ops):
            ...
            ax.axvline(np.median(data), color=colors[i], linestyle=':', linewidth=1)
    ...
    # Barres contribution %
    with col_bar:
        fig_bar, ax = plt.subplots(figsize=(3,3))
        contrib = [p_ev, p_av, p_vp]
        sources = ["EV","AV","VP"]
        ax.bar(sources, contrib, color=colors)
        for i, val in enumerate(contrib):
            ax.text(i, val+1, f"{val:.1f}%", ha='center', fontsize=8)
        ax.set_ylim(0, max(contrib)*1.2)
        ax.set_title("Contribution (%)", fontsize=9)
        ax.grid(True, linestyle='--', alpha=0.3)
    ...
    export_df = pd.DataFrame({
        "EV":[ev],"AV":[av],"VP":[vp],"GRR":[grr],"VT":[vt],
        "%EV":[p_ev],"%AV":[p_av],"%VP":[p_vp],"%GRR":[p_grr],"NdC":[ndc]
    })
    buffer = BytesIO()
