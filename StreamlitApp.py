...
if uploaded_file:
    ...
    if 'OP1-1' not in df.columns:
        ...
        col1, col2, col3 = st.columns(3)
        with col1:
            op1_cols = st.multiselect("Opérateur 1", df.columns, default=df.columns[:3] if len(df.columns) >= 3 else df.columns)
        with col2:
            op2_cols = st.multiselect("Opérateur 2", df.columns, default=df.columns[3:6] if len(df.columns) >= 6 else df.columns)
        with col3:
            op3_cols = st.multiselect("Opérateur 3", df.columns, default=df.columns[6:9] if len(df.columns) >= 9 else df.columns)
    else:
        op1_cols = ["OP1-1", "OP1-2", "OP1-3"]
        op2_cols = ["OP2-1", "OP2-2", "OP2-3"]
        op3_cols = ["OP3-1", "OP3-2", "OP3-3"]

    # Vérification que nous avons les colonnes nécessaires
    required_cols = op1_cols + op2_cols + op3_cols
    ...
    for col, (label, value, color, desc, icon) in zip([col1, col2, col3, col4], metrics):
        with col:
            percentage = (value/vt)*100 if label != "%GRR" and vt > 0 else value
    ...
    with col1:
        ...
        results_data = {
            ...
            "Valeur": [
                ...
                f"{(ev/vt*100):.2f}%" if vt > 0 else "0.00%",
                f"{(av/vt*100):.2f}%" if vt > 0 else "0.00%",
                f"{(vp/grr):.2f}" if grr > 0 else "0.00"
            ],
            "Statut": [
                "✅" if (ev/vt*100) < 30 else "⚠️" if vt > 0 else "-",
                "✅" if (av/vt*100) < 30 else "⚠️" if vt > 0 else "-",
                "✅" if p_grr < 10 else "⚠️" if p_grr <= 30 else "❌",
                "-", "-",
                "✅" if p_grr < 10 else "⚠️" if p_grr <= 30 else "❌",
                "✅" if (ev/vt*100) < 30 else "⚠️" if vt > 0 else "-",
                "✅" if (av/vt*100) < 30 else "⚠️" if vt > 0 else "-",
                "✅" if (vp/grr) >= 4 else "⚠️" if grr > 0 else "-"
            ]
        }
